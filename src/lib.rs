//! # Velotyp
//!
//! A Vello scene drawer for Typst's [`Frame`].

#![allow(
    clippy::cast_possible_truncation,
    reason = "Some castings are necessary for type conversion!"
)]
...
impl TypstScene {
    /// Create a new [`TypstScene`] from a [`Frame`].
    pub fn new(frame: &Frame) -> Self {
        let size = kurbo::Vec2::new(
            frame.size().x.to_pt(),
            frame.size().y.to_pt(),
        );
        let mut typst_scene = Self {
            size,
            ..Default::default()
        };
...
#[expect(
    ...
    reason = "vello::Scene does not implement debug."
)]
...
impl SceneKind {
    pub fn render(&self, scene: &mut vello::Scene) {
        match self {
            Self::Shape(shape) => shape.render(scene),
            Self::Text(text) => text.render(scene),
            Self::Image(image) => image.render(scene),
        };
    }
}
